...
| Tyyppi | `Tyyppi`, `Kind`, `Menetelmä` | `cluster` tekee rivistä cluster-liikkeen (ks. Sarjapainojen laskenta). Ilman saraketta rivi on cluster, jos huomautuksessa on sana *cluster*; huomautuksen `10 toiston maksimi` tai `10RM` kertoo viitetoistot (oletus 10) |
...
Rakenne etenee ylhäältä alas: **viikko** → **treenipäivä** → **liike**. "Lisää liike" avaa pohjalevyn, jonka hakukenttä suodattaa koko liikelistan nimen, lihasryhmän tai välineen perusteella — ensimmäisenä ryhmä "Ohjelmassa ja historiassa" (nimet, joilla on jo merkintöjä tai jotka ovat ohjelmassa) ja sen jälkeen sama lihasryhmittäin ryhmitelty katalogi variaatioineen kuin liikkeen vaihdossa. Enter valitsee ensimmäisen osuman. Jos liikettä ei löydy, kirjoitettua nimeä voi käyttää sellaisenaan; samankaltaisesta katalogin nimestä huomautetaan, koska eri kirjoitusasu aloittaisi historian alusta. Valinnan jälkeen liikkeelle annetaan sarjat, toistot, yksikkö ja teho, eli täsmälleen samat tiedot kuin CSV:n vastaavissa sarakkeissa; sarjat ja toistot kirjataan samalla sovelluksen omalla näppäimistöllä kuin Ohjelma-näkymässä, ja lomakkeen nimikentän napautus avaa liikelistan uudelleen.
...
Näkymä on sama ylhäältä alas etenevä rakenne kuin rakentajassa: viikot, niiden alla treenipäivät ja päivien alla liikkeet. Jokaisella rivillä on nuolet järjestyksen vaihtamiseen sekä ⋮-painike, jonka takaa löytyvät Kopioi, Aloita uudelleen ja Poista. Viikolle ja päivälle voi kirjoittaa oman nimen, esimerkiksi "Kevennysviikko" tai "Ylävartalo"; viikon oletusnimi on "Viikko 1", "Viikko 2" jne., ja nimen tyhjentäminen palauttaa oletuksen. Jokaisen liikkeen nimi näkyy vihreänä, kun liike on sama kuin liikepankissa tai liikehistoriassa (painoehdotukset ja Kehitys jatkuvat vanhoista tiedoista), ja messinkisenä huomiotekstillä "Liikettä ei tunnistettu liikepankista eikä historiasta — se lisätään uutena liikkeenä", kun nimeä ei voi yhdistää tunnettuun liikkeeseen; ruudunlukijalle tunnistus kerrotaan tekstinä. Liikettä napauttamalla avautuu lomake, jossa muutetaan nimi, sarjat, toistot, yksikkö ja teho — samat tiedot kuin CSV:n sarakkeissa; sama tunnistustieto näkyy lomakkeessa nimen alla heti valinnan jälkeen. Sarjat ja toistot kirjataan samalla sovelluksen omalla näppäimistöllä kuin Ohjelma-näkymässä (numerot, askelpalautin ja Seuraava, joka siirtyy Sarjoista Toistoihin ja sulkee Toistoista; laitteen näppäimistö ei avaudu), yksikkö ja teho laitteen näppäimistöllä. Nimeä ei kirjoiteta lomakkeeseen, vaan nimikentän napautus avaa liikevalitsimen: sen ensimmäinen ryhmä "Ohjelmassa ja historiassa" sisältää nimet, joilla on jo merkintöjä tai jotka ovat ohjelmassa (myös liikepankin nimet, jos niillä on historiaa), joten oikean nimen valinta yhdistää liikkeen vanhoihin historiatietoihin; sen jälkeen tulee koko liikepankki lihasryhmittäin, ja haku suodattaa nimen, lihasryhmän ja välineen mukaan. Valitsin näyttää nykyisen nimen, ja "Muokkaa nimeä" tuo sen hakukenttään esimerkiksi kirjoitusvirheen korjausta varten; listalta puuttuvan nimen voi ottaa käyttöön sellaisenaan. "Lisää liike" avaa saman liikevalitsimen kuin rakentajassa, "Lisää päivä" ja "Lisää tyhjä viikko" lisäävät uuden kohdan, ja "Kopioi viimeinen viikko uudeksi" monistaa viimeisen viikon päivineen ja liikkeineen seuraavaksi vapaaksi viikkonumeroksi.
...
Progressiokerroin on oletuksena 1,0125 eli tavoitteena 1,25 % kehitys jokaisella kirjauskerralla; koska tämä on kevyillä painoilla pienempi kuin pyöristysaskel, nosto on aina vähintään 2,5 kg. Esimerkkejä tavoitteella 10 toistoa: 100 kg × 10 → 102,5 kg, 60 kg × 10 → 62,5 kg, 100 kg × 12 → 105 kg (katto), 100 kg × 9 → 100 kg, 100 kg × 7 → 95 kg, 60 kg × 7 → 55 kg. Liikkeen tietopainikkeen ikkuna kertoo jokaisen sarjan perusteen.

**Tuntuma ehdotuksessa.** Työsarjalle annetaan tuntuma RPE-ikkunassa, joka avautuu sarjan ✓:stä ja rivin RPE-painikkeesta samalla asteikolla kuin lämmittelylle (valinta on vapaaehtoinen, näkyy lukuna RPE-painikkeessa ja poistuu ikkunan Poista tuntuma -painikkeesta; valinta sulkee ikkunan). Tuntuma tulkitaan RIR-arvoksi (reps in reserve, varastoon jääneet toistot) ja lisätään toistoihin Epley-arviossa; ehdotus on paino, jolla tavoitetoistot tuntuisivat Työläältä (RIR 2). Kun viime kerran sarjalle on kirjattu tuntuma, ehdotus lasketaan tällä kaavalla ilman edellä mainittuja nosto- ja kevennyshaaroja ja niiden rajoja:

```
1RM_edellinen = edellinen_paino × (1 + (edelliset_toistot + RIR) / 30)
1RM_tavoite   = 1RM_edellinen × progressiokerroin
paino         = MROUND( 1RM_tavoite / (1 + (tavoitetoistot + 2) / 30) , 2,5 )

RIR tuntuman mukaan: Kevyt 4, Sujuva 3, Työläs 2, Raskas 1, Äärirajoilla 0.
...
**Lämmittelyehdotus.** "Lisää lämmittelysarja" täyttää rivin valmiiksi painolla ja toistoilla. Ensisijaisesti käytetään viime kerran samaa lämmittelyä (L1 → L1, L2 → L2) skaalattuna tämän kerran työpainoon: jos viime kerran työsarja oli 100 kg ja lämmittely 60 kg × 5 ja tämän kerran ehdotus on 105 kg, lämmittely on 62,5 kg × 5. Jos viime kerralta ei ole vastaavaa lämmittelyä (tai se ei sovi nykyiseen työpainoon), paino tulee portaista 40 / 60 / 80 / 90 % työpainosta toistoilla 8 / 5 / 3 / 1; portaita suuremmat lämmittelyt toistavat viimeisen portaan. Alle 20 kg:n työpainolla ehdotetaan vain yksi lämmittely 60 %:lla ja tavoitetoistoilla, eikä alle 5 kg:n tai työpainon tasoista lämmittelyä ehdoteta. Työpaino on ensimmäisen työsarjan alkuperäinen ehdotus (ennen lämmittelysäätöä) tai käsin kirjoitettu paino; yhdistelmä- ja menetelmäliikkeille (esim. cluster) riviä ei esitäytetä. Lämmittely-otsikon alla oleva selite kertoo perusteen ("viime kerran mukaan, skaalattu työpainoon 105 kg" tai "ehdotettu työpainosta 100 kg (40 / 60 / 80 / 90 %)") ja katoaa, kun esitäytetyt rivit on merkitty valmiiksi tai muokattu. Ehdotus on deterministinen eikä opi, eikä se vaikuta työsarjoihin ennen kuin rivi on valmis.

**Lämmittelysäätö.** Liikkeelle voi lisätä lämmittelysarjoja, joille kirjataan paino, toistot ja tuntuma. Tuntuma valitaan RPE-ikkunassa, joka avautuu lämmittelyn ✓:stä ja rivin RPE-painikkeesta (painikkeet 6–10 sanoineen ja niiden alla asteikon ohje); asteikko vastaa kysymykseen, montako toistoa olisi vielä jaksanut: 6 Kevyt (neljä tai enemmän), 7 Sujuva (kolme), 8 Työläs (kaksi), 9 Raskas (yksi) ja 10 Äärirajoilla (ei yhtään). Uusi lämmittelysarja tulee aiempien lämmittelyjen perään (L1, L2, …). Sisäisesti pykälät tallennetaan RPE-lukuina 6–10, joten laskenta ja aiemmat kirjaukset ovat ennallaan. Ensin tarkistetaan, onko valmiiksi merkitty lämmittely tosiasiassa työsarjan tasoinen: paino vähintään ensimmäisen työsarjan ehdotettu paino ja toistot vähintään liikkeen tavoitetoistot. Silloin vertailudataa ei tarvita, vaan jokainen vielä merkitsemätön ja käsin muokkaamaton työsarja saa painon samalla tuntumakaavalla ilman progressiota (`paino = MROUND( lämmittelypaino × (1 + (lämmittelytoistot + RIR) / 30) / (1 + (tavoitetoistot + 2) / 30) , 2,5 )`; ilman tuntumaa RIR = 2). Esimerkiksi tavoitteella 12 toistoa ja ehdotuksella 162,5 kg lämmittely 162,5 kg × 12 antaa Kevyellä 170 kg, Sujuvalla 165 kg, Työläällä 162,5 kg (paino pidetty), Raskaalla 160 kg ja Äärirajoilla 155 kg. Selite kertoo säädetyt sarjat ("sarjat 2–3 säädetty 162,5 → 170 kg" tai "paino pidetty"); tuntuman vaihto lasketaan aina alkuperäisestä ehdotuksesta, ja lämmittelyrivi säilyy lämmittelynä datassa ja Historiassa. Muuten saman treenin painavin valmiiksi merkitty lämmittely, jolle on annettu tuntuma, verrataan liikkeen aiempiin lämmittelyihin, joissa paino on 2,5 kg:n sisällä ja toistot samat. Jos vertailudataa ei vielä ole, työsarjoja ei säädetä ja selite kertoo tämän. Muuten aiempien tuntumien keskiarvosta lasketaan ero: vähintään kaksi pykälää raskaampi → työsarjat −10 %, vähintään yksi pykälä raskaampi → −5 %, vähintään yksi pykälä kevyempi → +2,5 %, muuten ennallaan; paino pyöristetään alaspäin 2,5 kg:n askeleeseen. Säätö koskee vain progressioehdotuksen tuottamia rivejä, joita ei ole muokattu käsin, ja vain tehottomia liikkeitä. Lämmittelyt tallentuvat merkinnän mukana ja liikkeen kolmen viimeisimmän kerran historiaan, mutta ne eivät vaikuta Historian ja Kehityksen laskentaan, eivät käynnistä lepoajastinta. Varmuuskopio sisältää lämmittelyt työsarjojen jälkeen riveinä L1, L2, … painoineen, toistoineen ja tuntumineen (ks. Varmuuskopio).

Sovellus muistaa liikkeen kolme viimeisintä kertaa. **Jumitunnistus ja kevennys.** Jos käytetty maksimipaino ei ole noussut kolmen kerran aikana ja viimeisimmällä kerralla vähintään yksi sarja jäi vajaaksi, sovellus ehdottaa kevennystä ja selite kertoo syyn. Ilman tuntumaa vaje tarkoittaa tavoitetta vähempiä toistoja ja kevennys on 10 prosenttia edellisen kerran sarjan painosta alaspäin pyöristäen. Tuntuman kanssa sovelletaan samaa RIR-sääntöä kuin ehdotuksessa: vaje mitataan toistoista ja varastosta yhteensä, ja kevennys on 90 prosenttia tuntuman mukaisesta arviosta tavoitetoistoille alaspäin pyöristäen, jolloin selite lisää "tuntuma huomioiden".
...
Sama koskee PDF-tuonnin menetelmäliikkeitä (rest-pause, drop) sekä kiertoharjoittelu- ja kestorivejä: niille painoa ei ehdoteta lainkaan, koska tavoite ei ole yksiselitteinen sarjamäärä × toistot ja väärä ehdotus olisi huonompi kuin tyhjä kenttä. Kiertoharjoittelu- ja kestoriveillä sarjan voi merkitä valmiiksi ilman painoa.

**Cluster-sarjat.** Cluster on vakiokuormalla tehty ketju lyhyitä osasarjoja (esim. "10 toiston maksimikuormalla 6 toiston sarjoja 10–15 sekunnin tauoilla niin monta kuin saat, kaksi settiä"). Liike on cluster-tyyppinen, kun PDF:n tavoite on `2xCluster` tai CSV:n Tyyppi-sarakkeessa on `cluster` (tai sarakkeen puuttuessa huomautuksessa on sana cluster). Yksi cluster kirjataan yhtenä sarjarivinä, jolla on paino, osasarjan toistot ja osasarjojen lukumäärä. Paino esitäytetään **viitekuormalla**: liikkeen viimeisin tavallinen merkintä, jossa jokin sarja tehtiin täsmälleen viitetoistoilla (oletus 10; ohjelman huomautuksen tai PDF-selitteen `N toiston maksimi` / `NRM` muuttaa sen), ja kuorma on sen merkinnän näiden sarjojen suurin paino. Jos sellaista merkintää ei ole, paino jää tyhjäksi ja tietopainikkeen ikkuna kehottaa syöttämään sen käsin. Toistot ovat ohjelman toistotavoite ja osasarjat alkavat nollasta; rivin alla oleva laskuri lisää (+) tai poistaa (−) osasarjan, ja + käynnistää 15 sekunnin osasarjatauon ajastimen aina alusta (sama äänimerkki ja asetus kuin lepoajastimella). Cluster merkitään valmiiksi vasta, kun painossa on arvo ja osasarjoja on vähintään yksi; rivin ✓ käynnistää tavallisen lepoajastimen paitsi viimeisestä rivistä. Kehityksen mittari on osasarjojen määrä samalla kuormalla: selite kertoo kuorman lähteen ja edellisen cluster-kerran osasarjat ("Viimeksi 5 + 4 osasarjaa — tavoitteena ylittää"). Cluster-merkintä ei vaikuta Kehityksen 1RM-arvioon eikä ennätyksiin (6 toiston osasarja 10RM-kuormalla antaisi Epleyllä keinotekoisen notkahduksen) eikä päivitä 1RM-arvoa, mutta sen volyymi on kaikkialla paino × toistot × osasarjat. Saman liikkeen cluster-kertaa ei käytetä tavallisen sarjan painoehdotuksen viitekertana.
...
**Ohjelma.** Näkymän otsikkona on viikon nimi ja sen vieressä nuolet edelliseen ja seuraavaan viikkoon sekä laskuri (esim. "1 / 4"); Kaikki-tilassa otsikko on "Koko ohjelma" ja jokaisen viikon päivät ovat oman väliotsikon alla. Näkymän ylimpänä on Seuraavaksi-kortti, joka kertoo vuorossa olevan treenipäivän ja liikkeen, avatun liikkeen sarjatilanteen ("sarja 3 / 4"), tallennettujen liikkeiden osuuden palkkina ja edellisen kirjauspäivän; sen "Aloita"- tai "Jatka"-painike avaa päivän ja sen ensimmäisen kirjaamattoman liikkeen, ja kun viikon kaikki päivät on tehty, kortti kertoo sen ja tarjoaa siirtymän seuraavaan viikkoon. Treenipäivät ovat kortteina, joissa on päivän nimi ja tila (tehty, kesken, liikkeiden määrä); kortti avaa päivän. Kapealla näytöllä liike avautuu omaksi kirjausruudukseen: yläreunassa "‹ Ohjelma" -painike, sen alla treenipäivä ja liikkeen järjestysnumero, liikkeen nimi, tavoiterivi ja koko sarjataulukko; laitteen taaksepäin-ele, "‹ Ohjelma" ja välilehden vaihto sulkevat ruudun, kesken jäänyt luonnos säilyy ja palaa Seuraavaksi-kortin Jatka-painikkeesta, ja tallennuksen jälkeen ruutu vaihtuu päivän seuraavaan liikkeeseen (viimeisen jälkeen palataan ohjelmaan). Tabletilla avattu liike näkyy oikean palstan paneelissa ja tallennuksen jälkeen seuraava liike nousee esiin. Avatun päivän liikkeet on ryhmitelty päivän kortin alle. Liikkeen otsikkona on liikepankin liikenimi ja sen alla pienellä väline, variaatio ja lihasryhmä (esim. "Kulmasoutu tangolla" ja "Tanko · Trap bar · Selkä"); liikepankin ulkopuolinen nimi näytetään sellaisenaan. Liikkeen avaamalla kirjaat painot ja toistot sarjoittain yhdellä rivillä per sarja (sarakkeet Sarja, Viime, Kg, Toistot ja ✓; Viime näyttää kahdella rivillä edellisen kerran painon ja toistot, esim. 60×10, ja niiden alla sarjan RPE:n, jos se oli merkitty; sarake jää alle 375 px:n näytöillä pois näkyvistä tilan puutteen vuoksi). Painon tai toistojen napautus avaa sovelluksen oman näppäimistön näytön alareunaan laitteen näppäimistön sijaan: numerot, pilkku (vain painoille), askelpalautin, painoille ±2,5 kg -painikkeet, ✓ (merkitsee kohdesarjan valmiiksi poistumatta näppäimistöltä, sama toiminto kuin rivin ✓) ja Seuraava, joka siirtyy seuraavan sarjan samaan kenttään ja viimeisestä sarjasta sulkee näppäimistön; nuoli sulkee sen milloin tahansa, ja fyysinen näppäimistö toimii ennallaan (Enter = Seuraava, Esc sulkee). Avattu liike vieritetään aina näytön yläreunaan, ja kentän fokus, kursorin paikka ja vieritysasema säilyvät, vaikka näkymä piirretään uudelleen kesken kirjoituksen (esim. "+ Sarja" tai ✓); huomiokentän ja muiden laitteen näppäimistöä käyttävien kenttien fokus tuo kentän näkyvän alueen keskelle ja piilottaa alavalikon näppäimistön ajaksi. ✓ merkitsee sarjan tehdyksi (reunaton messinkinen merkki muuttuu vihreäksi, sarjanumero säilyy), ja "Tallenna merkintä" näkyy harmaana ääriviivana, kunnes kaikki sarjat on merkitty. Jo tallennetun merkinnän voi avata ja korjata myöhemmin, ja korjaus säilyy alkuperäisellä treenipäivällä. Sarjojen alla on koko rivin levyinen "+ Sarja" ja sen alla tekstipainike "Lisää lämmittelysarja", joka lisää lämmittelyrivin valmiiksi täytettynä (viime kerran sama lämmittely skaalattuna työpainoon tai portaat 40 / 60 / 80 / 90 %, ks. Sarjapainojen laskenta) — paino ja toistot ovat muokattavissa ja tuntuma valitaan lämmittelyn ✓:n avaamassa RPE-ikkunassa, ja kun sama lämmittely on kirjattu aiemmin, poikkeava tuntuma säätää saman treenin työsarjojen painoehdotusta (ks. Sarjapainojen laskenta); ±2,5 kg -painikkeet tuovat tyhjään kenttään ensin viime kerran painon; sarjan ✓ avaa RPE-ikkunan, jossa valitaan sarjan tuntuma (RPE 6–10, valinnainen, ks. Sarjapainojen laskenta) painikkeista sanoineen tai joka suljetaan ilman valintaa; lepoajastin käynnistyy vasta ikkunan sulkeuduttua suoraan pienenä palkkina, ja rivin RPE-painike avaa ikkunan uudelleen — siellä ovat myös asteikon ohje, tuntuman poisto ja sarjan poisto (erillistä huomiokenttää ei ole; aiemmin kirjatut huomiot näkyvät Historiassa). Avoimen liikkeen nimen perässä on tietopainike (ⓘ, ilman kehystä ja täyttöä), joka avaa ikkunan painojen laskennasta: mihin kertaan ehdotus perustuu, mikä on kunkin sarjan peruste, lämmittelysäädön tila, maksimitestin ohje (nosto tallentuu liikkeen 1RM:ksi) ja valmentajan menetelmäselite; samat tiedot eivät näy sarjarivien yläpuolella. Tallennuksen ilmoitus näyttää uuden 1RM:n. Cluster-liikkeen rivillä on lisäksi osasarjalaskuri (−, lukumäärä ja +; painikkeet ovat samaa 44 px:n kokoa kuin rivin ✓) sarjarivin alla, ja tallennettu cluster näyttää suljettuna kuorman ja osasarjat ("Kirjattu: 60 kg · 5 + 4 osasarjaa"). Yläreunan "Viikko | Kaikki" -valitsin näyttää nykyisen viikon tai koko ohjelman, ja päivämääräsirpaleesta merkinnän voi kirjata myös takautuvasti. Treenipäivien alapuolella "Kirjaa vaiva" avaa vaivalokin lomakkeen samalle kirjauspäivälle (ks. Vaivaloki). Oikean yläkulman kynäkuvake avaa ohjelman muokkaustilan (ks. Ohjelman muokkaus sovelluksessa).
...
**Historia.** Merkinnät päivittäin; ennen ensimmäistä merkintää näkymä kertoo, mistä aloittaa, ja vie painikkeella Ohjelmaan. jokainen päiväkortti näyttää jo suljettuna liikkeiden määrän ja nostetun kokonaispainon. Avattu päivä listaa liikkeet sarjoineen (esim. "1: 60 kg × 10"), ja sarjan alla näkyy "RPE 8", jos sarjalle oli merkitty tuntuma. Yksittäisen päivän voi poistaa kortin alareunasta kahdella painalluksella, jolloin painoehdotukset ja Kehitys-näkymän laskennalliset maksimit lasketaan uudelleen jäljellä olevasta historiasta.

**Kehitys.** Etusivulla on kolme osiota: Yhteenveto vastaa kysymykseen ”miten menee”, Liikkeet näyttää haun ja liikelistan heti ylhäältä ja Vaivat kokoaa vaivalokin; valinta säilyy, kun liikenäkymästä palataan tai välilehteä vaihdetaan. Viikko-kortti näyttää kuluvan treeniviikon (tai jos sillä ei ole merkintöjä, viimeisimmän treeniviikon) kokonaispainon, sarjat, toistot ja treenipäivät 2 × 2 -ruudukkona sekä erot edelliseen merkintäviikkoon neutraalilla värillä, ja sen alla kahdentoista viikon volyymipylväät (kortin viikko messinkinä, kevennysviikot harmaina, viikot ilman merkintöjä ohuena viivana); ”Näytä liikkeet” avaa viikon liikkeet sarjoineen, toistoineen ja kiloineen tonnagen mukaan järjestettynä, yhdistelmäliikkeet lopuksi. Nostettu kokonaispaino -kortissa on yksi käyrä, jonka karkeuden voi vaihtaa viikon ja päivän välillä (Päivä on aiempi treenipäivän kokonaispaino sellaisenaan). Ohjelman toteutuminen -kortti kertoo viimeiseltä 28 päivältä mittaririveinä ja palkkeina tehdyt sarjat suhteessa ohjelmoituihin, kuinka usein tavoitetoistot täyttyivät ja kuinka usein 1,25 %:n progressiotavoite toteutui, sekä pysähtyneet liikkeet napautettavina tunnisteina (1RM ei ole noussut kolmeen viimeisimpään kertaan). Liikkeet-osiossa on aakkosjärjestetty liikelista, jonka rivillä on liikkeen nimi, viimeisin päivä, laskennallinen 1RM ja ero edelliseen kertaan; yli kahdeksan liikkeen listaa voi suodattaa hakukentällä. Yhdistelmäliikkeet (nimessä kauttaviiva) eivät ole listassa, mutta ne lasketaan viikon sarja- ja toistomääriin.
...
| Osio | Sisältö |
|---|---|
| `#MERKINNÄT` | Kirjatut sarjat: päivämäärä, liike, sarjanumero, paino, toistot, huomiot, tyyppi, liikkeen tunniste, tuntuma (asteikon sana, tyhjä jos ei annettu) ja osasarjat (cluster-rivillä osasarjojen määrä, muilla tyhjä) |
| `#OHJELMA` | Kaikkien ohjelmien jokainen liike viikkoineen ja treenipäivineen, mukaan lukien tyyppi, menetelmä, sarjakohtaiset toistot ja menetelmäselite sekä viikon nimi, päivän tunniste ja päivän nimi; ohjelman tunniste, nimi ja Käytössä-sarake erottavat ohjelmat toisistaan; viimeisenä Viitetoistot cluster-liikkeen viitetoistoille |
...
Merkintärivin **Tuntuma** on sarjan tuntuma asteikon sanana (Kevyt … Äärirajoilla); tuonti hyväksyy myös RPE-luvun 6–10 ja tiedoston ilman saraketta, jolloin tuntuma jää tyhjäksi. Liikkeen lämmittelysarjat ovat työsarjojen jälkeen omina riveinään, joiden **Sarja** on L1, L2, … (paino, toistot ja tuntuma; Huomiot ja Osasarjat tyhjiä); tuonti liittää ne edeltävään liikkeeseen, ja vanha tiedosto ilman L-rivejä tuodaan ennallaan. Merkintärivin **Osasarjat** on cluster-rivin osasarjojen määrä ja **Tyyppi** cluster-merkinnällä `cluster`; ilman Osasarjat-saraketta tuotu cluster-rivi tulkitaan yhdeksi osasarjaksi, ja kaksoiskappaleen tunnistus erottaa rivit myös pelkän osasarjamäärän perusteella. Merkintärivin **Tunniste** on liikkeen ohjelma-id. Sen ansiosta palautus sitoo jokaisen merkinnän täsmälleen samaan ohjelman liikkeeseen kuin varmuuskopion hetkellä, eikä liikkeiden nimien varaan jäävää arvausta tarvita. Vanhemmissa, ennen tätä tehdyissä vientitiedostoissa saraketta ei ole; ne tuodaan yhä entiseen tapaan nimien perusteella kohdistaen.
