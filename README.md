...
**Kehitys.** Jokaiselle liikkeelle 1RM parhaana arvona viimeiseltä neljältä viikolta (mitattu tai laskennallinen) sekä muutos kuukauteen, puoleen vuoteen, vuoteen ja ennätykseen verrattuna — kiloina ja prosentteina, sekä SVG-viivakäyränä ajan yli, jossa päiväkohtaiset arviot ovat katkoviivana. Liike näkyy heti, kun sille on kirjattu yksikin merkintä, josta 1RM voidaan laskea; vertailurivit näkyvät vasta, kun vertailukohta on olemassa, ja yhdellä merkinnällä kortti kertoo, että toinen merkintä tarvitaan. Ylimpänä myös vastaava käyrä treenipäivän nostetulle kokonaispainolle.
