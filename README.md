...
Ohjelma-välilehden "Kirjaa vaiva" -painike kirjaa kehonosakohtaisen vaivan valitulle kirjauspäivälle: kehonosa (niska, olkapää, kyynärpää, ranne, alaselkä, lonkka, polvi, nilkka tai muu), parillisilla kehonosilla puoli (vasen, oikea tai molemmat), voimakkuus (lievä, kohtalainen tai voimakas) ja vapaa huomio. Kirjaukset näkyvät Kehitys-välilehden Vaivat-osiossa uusin ensin, ja yksittäisen kirjauksen voi poistaa kahdella painalluksella.
...
**Ohjelma.** Treenipäivät kortteina, joissa on päivän nimi, tila (tehty, kesken, liikkeiden määrä) ja seuraavaksi vuorossa olevalla päivällä "Aloita"-painike, joka avaa päivän ja sen ensimmäisen kirjaamattoman liikkeen. Liikkeen otsikkona on liikepankin liikenimi ja sen alla pienellä väline, variaatio ja lihasryhmä (esim. "Kulmasoutu tangolla" ja "Tanko · Trap bar · Selkä"); liikepankin ulkopuolinen nimi näytetään sellaisenaan. Liikkeen avaamalla kirjaat painot ja toistot sarjoittain yhdellä rivillä per sarja; jo tallennetun merkinnän voi avata ja korjata myöhemmin, ja korjaus säilyy alkuperäisellä treenipäivällä; "Lisää lämmittelysarja" lisää lämmittelyrivin, jolle kirjataan paino, toistot ja tuntuma (RPE 6–10), ja kun sama lämmittely on kirjattu aiemmin, poikkeava tuntuma säätää saman treenin työsarjojen painoehdotusta (ks. Sarjapainojen laskenta); ±2,5 kg -säätimet näkyvät aktiivisen sarjan alla ja sarjan huomio sekä poisto löytyvät rivin ⋮-painikkeesta. Yläreunan "Viikko | Kaikki" -valitsin näyttää nykyisen viikon tai koko ohjelman, ja päivämääräsirpaleesta merkinnän voi kirjata myös takautuvasti. Sen alla "Kirjaa vaiva" avaa vaivalokin lomakkeen samalle kirjauspäivälle (ks. Vaivaloki). Oikean yläkulman kynäkuvake avaa ohjelman muokkaustilan (ks. Ohjelman muokkaus sovelluksessa).
...
**Kehitys.** Jokaiselle liikkeelle 1RM parhaana arvona viimeiseltä neljältä viikolta (mitattu tai laskennallinen) sekä muutos kuukauteen, puoleen vuoteen, vuoteen ja ennätykseen verrattuna — kiloina ja prosentteina, sekä SVG-viivakäyränä ajan yli, jossa päiväkohtaiset arviot ovat katkoviivana. Saman liikkeen eri variaatiot (eri tanko tai kahva) lasketaan yhteen liikkeen nimen alle, ja kortti kertoo, millä variaatioilla liike on kirjattu; sama sääntö koskee vaivalokin analyysiä. Liike näkyy heti, kun sille on kirjattu yksikin merkintä, josta 1RM voidaan laskea; vertailurivit näkyvät vasta, kun vertailukohta on olemassa, ja yhdellä merkinnällä kortti kertoo, että toinen merkintä tarvitaan. Ylimpänä myös vastaava käyrä treenipäivän nostetulle kokonaispainolle. Alimpana Vaivat-osio: kirjatut vaivat kehonosittain, vähintään kolmen kirjauksen jälkeen liikkeet, joiden volyymi ylitti toistuvasti tavanomaisen vaivaa edeltävällä viikolla, sekä kirjausluettelo poistopainikkeineen (ks. Vaivaloki).
